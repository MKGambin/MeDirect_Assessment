...
{
    [TestFixture]
    public class AppUserHelperTests
    {
        ...
        #region ValidAppUserAsync
        [Test]
        public async Task ValidAppUserAsync_ShouldReturnTrue_WhenUserExists()
        {
            var result = await AppUserHelper.ValidAppUserAsync(context: _context, identifier: "1b7d4e2c-8f6a-49a1-92f5-3d7c8b6f2e4a");

            Assert.That(result, Is.True);
        }

        [Test]
        public async Task ValidAppUserAsync_ShouldReturnFalse_WhenUserDoesNotExist()
        {
            var result = await AppUserHelper.ValidAppUserAsync(context: _context, identifier: "identifier-invalid");

            Assert.That(result, Is.False);
        }
        #endregion
        #region GetAppUserDetailsAsync
        [TestCase(arg: -1, Description = "Invalid Record")]
        public async Task GetAppUserDetailsAsync_ShouldThrowInvalidOperationException_WhenUserNotFound(int appUserId)
        {
            try
            {
                var dbItem = await _context.AppUsers.FirstOrDefaultAsync(x => x.Id == appUserId);
                var dbItemIdentifier = dbItem?.Identifier ?? "";

                await AppUserHelper.GetAppUserDetailsAsync(context: _context, identifier: dbItemIdentifier);
            }
            catch (Exception ex)
            {
                Assert.That(ex.Message, Is.EqualTo("AppUser is invalid."));
            }
        }

        [Test]
        public async Task GetAppUserDetailsAsync_ShouldReturnCorrectUserDetails_WhenUserExists()
        {
            var appUserId = 1;
            var appUserIdentifier = "e3a4f2c6-63b8-4d21-9eb8-06545e8f1a91";

            var result = await AppUserHelper.GetAppUserDetailsAsync(context: _context, identifier: appUserIdentifier);
            dynamic resultObj = result;

            var appUser = await _context.AppUsers.FirstOrDefaultAsync(x => x.Id == appUserId);
            var appUserCurrentBalance = await AppUserHelper.GetAppUserCurrentBalanceAsync(context: _context, identifier: appUserIdentifier);
            var appUserLastTradeExecuted = await AppUserHelper.GetAppUserLastExecutedTradeAsync(context: _context, identifier: appUserIdentifier);

            Assert.That(resultObj, Is.Not.Null);
            Assert.That(resultObj.Identifier, Is.EqualTo(appUser?.Identifier));
            Assert.That(resultObj.FirstName, Is.EqualTo(appUser?.FirstName));
            Assert.That(resultObj.LastName, Is.EqualTo(appUser?.LastName));
            Assert.That(resultObj.Email, Is.EqualTo(appUser?.Email));
            Assert.That(resultObj.CreatedOn, Is.EqualTo($"{appUser?.CreatedOn:dd-MM-yyyy}"));
            Assert.That(resultObj.CurrentBalance, Is.EqualTo($"{appUserCurrentBalance:F2}"));
            Assert.That(resultObj.LastTradeExecutedOn, Is.EqualTo($"{appUserLastTradeExecuted?.ExecutedOn:dd-MM-yyyy}"));
        }
        #endregion
        #region GetAppUserCurrentBalanceAsync
        [TestCase(arg: -1, Description = "Invalid Record")]
        public async Task GetAppUserCurrentBalanceAsync_ShouldThrowInvalidOperationException_WhenUserNotFound(int appUserId)
        {
            try
            {
                var dbItem = await _context.AppUsers.FirstOrDefaultAsync(x => x.Id == appUserId);
                ...
                await AppUserHelper.GetAppUserCurrentBalanceAsync(context: _context, identifier: dbItemIdentifier);
            }
            catch (Exception ex)
            {
                Assert.That(ex.Message, Is.EqualTo("AppUser is invalid."));
        ...
        [Test]
        public async Task GetAppUserCurrentBalanceAsync_ShouldReturnCorrectBalance_WhenUserHasExecutedTrades()
        {
            var appUserId = 1;
            var appUserIdentifier = "e3a4f2c6-63b8-4d21-9eb8-06545e8f1a91";

            var result = await AppUserHelper.GetAppUserCurrentBalanceAsync(context: _context, identifier: appUserIdentifier);

            var appUserBalance = await _context.Trades
                .Where(x => x.AppUserId == appUserId && x.Status == TradeStatus.Executed)
                .SumAsync(x => x.TotalAmount);

            Assert.That(result, Is.EqualTo(appUserBalance));
        }
        #endregion
        #region GetAppUserLastExecutedTradeAsync
        [TestCase(arg: -1, Description = "Invalid Record")]
        public async Task GetAppUserLastExecutedTradeAsync_ShouldThrowInvalidOperationException_WhenUserNotFound(int appUserId)
        {
            try
            {
                var dbItem = await _context.AppUsers.FirstOrDefaultAsync(x => x.Id == appUserId);
                ...
                await AppUserHelper.GetAppUserLastExecutedTradeAsync(context: _context, identifier: dbItemIdentifier);
            }
            catch (Exception ex)
            {
                Assert.That(ex.Message, Is.EqualTo("AppUser is invalid."));
        ...
        [Test]
        public async Task GetAppUserLastExecutedTradeAsync_ShouldReturnLastExecutedTrade_WhenUserHasExecutedTrades()
        {
            var appUserId = 1;
            var appUserIdentifier = "e3a4f2c6-63b8-4d21-9eb8-06545e8f1a91";

            var result = await AppUserHelper.GetAppUserLastExecutedTradeAsync(context: _context, identifier: appUserIdentifier);

            var appUserLastExecutedTrade = await _context.Trades
                .Where(x => x.AppUserId == appUserId && x.Status == TradeStatus.Executed)
                .OrderByDescending(x => x.ExecutedOn)
                .FirstOrDefaultAsync();

            Assert.That(result, Is.Not.Null);
            Assert.That(result.Id, Is.EqualTo(appUserLastExecutedTrade?.Id));
        }
        #endregion
        #region GetAppUserTradesAsync
        [TestCase(arg: -1, Description = "Invalid Record")]
        public async Task GetAppUserTradesAsync_ShouldThrowException_WhenUserNotFound(int appUserId)
        {
            try
            {
                var dbItem = await _context.AppUsers.FirstOrDefaultAsync(x => x.Id == appUserId);
                ...
                await AppUserHelper.GetAppUserTradesAsync(context: _context, identifier: dbItemIdentifier);
            }
            catch (Exception ex)
            {
                Assert.That(ex.Message, Is.EqualTo("AppUser is invalid."));
        ...
        [Test]
        public async Task GetAppUserTradesAsync_ShouldReturnCorrectTradeCount()
        {
            var appUserId = 1;
            var appUserIdentifier = "e3a4f2c6-63b8-4d21-9eb8-06545e8f1a91";

            var result = await AppUserHelper.GetAppUserTradesAsync(context: _context, identifier: appUserIdentifier);
            dynamic resultObj = result;

            var appUserTradesCount = await _context.Trades.CountAsync(x => x.AppUserId == appUserId);

            Assert.That(resultObj.TotalTrades, Is.EqualTo(appUserTradesCount));
        }

        [Test]
        public async Task GetAppUserTradesAsync_ShouldReturnCorrectTradeIdentifiers()
        {
            var appUserId = 1;
            var appUserIdentifier = "e3a4f2c6-63b8-4d21-9eb8-06545e8f1a91";
            ...
            var appUserTradesIdentifiers = await _context.Trades.Where(x => x.AppUserId == appUserId).Select(x => x.Identifier).ToListAsync();

            var resultObjTradesIdentifiers = ((IEnumerable<dynamic>)resultObj.Trades).Select(x => (string)x.Identifier).ToList();

            Assert.That(resultObjTradesIdentifiers, Is.EquivalentTo([.. appUserTradesIdentifiers]));
        }
        #endregion
        #region CreateTrade
        [TestCase(arg: -1, Description = "Invalid Record")]
        public async Task CreateTrade_ShouldThrowException_WhenUserNotFound(int appUserId)
        {
            try
            {
                var dbItem = await _context.AppUsers.FirstOrDefaultAsync(x => x.Id == appUserId);
                ...
                await AppUserHelper.CreateTrade(context: _context, rabbitMQHelper: null, identifier: dbItemIdentifier, tradeAction: TradeAction.In, price: 1.00m);
            }
            catch (Exception ex)
            {
                Assert.That(ex.Message, Is.EqualTo("AppUser is invalid."));
        ...
        [TestCase(1, TradeAction.In, 200.00)]
        [TestCase(2, TradeAction.Out, 150.00)]
        public async Task CreateTrade_ShouldCreateTradeWithCorrectDetails(int appUserId, TradeAction tradeAction, decimal price)
        {
            var dbItem = await _context.AppUsers.FirstOrDefaultAsync(x => x.Id == appUserId);
            var dbItemIdentifier = dbItem?.Identifier ?? "";

            await AppUserHelper.CreateTrade(context: _context, rabbitMQHelper: null, identifier: dbItemIdentifier, tradeAction: tradeAction, price: price);

            var trade = await _context.Trades.OrderByDescending(x => x.Id).FirstOrDefaultAsync();

            var expectedQuantity = tradeAction == TradeAction.In ? 1 : -1;
            var expectedTotalAmount = expectedQuantity * price;

            Assert.That(trade, Is.Not.Null);
            Assert.That(trade.AppUserId, Is.EqualTo(appUserId));
            Assert.That(trade.Action, Is.EqualTo(tradeAction));
            Assert.That(trade.Quantity, Is.EqualTo(expectedQuantity));
            Assert.That(trade.Price, Is.EqualTo(price));
            Assert.That(trade.TotalAmount, Is.EqualTo(expectedTotalAmount));
            Assert.That(trade.Status, Is.EqualTo(TradeStatus.Pending));
        }
        #endregion
    }
}
